...
- Evidence reviewed: `README.md`, `docs/architecture/overview.md`, `docs/architecture/blender-companion.md`, `docs/adr/0010-paginated-revision-history.md`, `packages/protocol/src/guide.ts`, `packages/protocol/src/proposal.ts`, `packages/protocol/src/revision.ts`, `packages/protocol/src/diff.ts`, `packages/protocol/src/history.ts`, `packages/protocol/src/catalog.ts`, `packages/protocol/src/eval.ts`, `services/orchestrator/src/index.ts`, `services/orchestrator/src/guide-plan-diff.ts`, `services/orchestrator/src/guide-revision-history.ts`, `services/orchestrator/src/eval-export.ts`, `packages/persistence/src/index.ts`, `adapters/blender/catalog/v1/action-catalog.json`, `adapters/blender/extension/operating_line/application/companion.py`, `adapters/blender/extension/operating_line/application/revision_review.py`, `adapters/blender/extension/operating_line/application/session.py`, `adapters/blender/extension/operating_line/presentation/panel.py`, `adapters/blender/extension/operating_line/presentation/operators.py`, `adapters/blender/extension/operating_line/infrastructure/overlay.py`, `adapters/blender/extension/operating_line/resources/snowman.plan.json`, `tests/e2e/blender/capture_overlay.py`, and `docs/assets/blender-guidance.png`.
- Product boundary: the shipped baseline is a deterministic 15-step Blender snowman vertical slice, including bounded rigid armature binding and pose keyframes, plus model-neutral ActionCatalog/PlanningContext, human-approved GuideProposal, node references, immutable linear revision threads, deterministic Plan diffs, paginated structured revision history, and versioned raw Eval/replay export. A Blender user may reference nodes in an active or proposed tree and queue a change request for Codex, Claude, or another MCP client; a replan always returns as a complete new GuideProposal revision, and only an accepted request-linked proposal can become the exact base of the next turn. OperatingLine does not embed a model, stream a chat response, preserve model reasoning, infer a quality score, or automatically sanitize exported evidence. Parameter-form editing, explicit branching/merging, scoring/training governance, deform/weight-painted rigs, and a second software host remain later milestones.
...
- Token/component ownership: host-neutral step/anchor, proposal/decision, node-reference, revision-thread, Plan-diff, revision-request, revision-history page, and Eval bundle semantics stay in `packages/protocol`; proposal/revision-request persistence, thread uniqueness, idempotency, and stable event sequencing stay in `packages/persistence`; thread continuity and accepted-parent validation, deterministic diff computation, history joins/pagination, Eval relation filtering, summaries, and content hashing stay in the Orchestrator; deterministic accepted-session, proposal-preview, diff/history validation, page merging, and local composer lineage stay in Blender application code; Blender RGBA and drawing constants stay in Blender presentation/infrastructure code.
