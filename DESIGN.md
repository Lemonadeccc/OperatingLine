...
- Evidence reviewed: `README.md`, `docs/architecture/overview.md`, `docs/architecture/blender-companion.md`, `packages/protocol/src/guide.ts`, `packages/protocol/src/proposal.ts`, `packages/protocol/src/revision.ts`, `packages/protocol/src/diff.ts`, `packages/protocol/src/catalog.ts`, `packages/protocol/src/eval.ts`, `services/orchestrator/src/index.ts`, `services/orchestrator/src/guide-plan-diff.ts`, `services/orchestrator/src/eval-export.ts`, `packages/persistence/src/index.ts`, `adapters/blender/catalog/v1/action-catalog.json`, `adapters/blender/extension/operating_line/application/companion.py`, `adapters/blender/extension/operating_line/application/revision_review.py`, `adapters/blender/extension/operating_line/application/session.py`, `adapters/blender/extension/operating_line/presentation/panel.py`, `adapters/blender/extension/operating_line/presentation/operators.py`, `adapters/blender/extension/operating_line/infrastructure/overlay.py`, `adapters/blender/extension/operating_line/resources/snowman.plan.json`, `tests/e2e/blender/capture_overlay.py`, and `docs/assets/blender-guidance.png`.
- Product boundary: the shipped baseline is a deterministic 15-step Blender snowman vertical slice, including bounded rigid armature binding and pose keyframes, plus model-neutral ActionCatalog/PlanningContext, human-approved GuideProposal, node references, immutable linear revision threads, deterministic Plan diffs, and versioned raw Eval/replay export. A Blender user may reference nodes in an active or proposed tree and queue a change request for Codex, Claude, or another MCP client; a replan always returns as a complete new GuideProposal revision, and an accepted request-linked proposal can become the exact base of the next turn. OperatingLine does not embed a model, stream a chat response, show a complete conversation history, infer a quality score, or automatically sanitize exported evidence. Parameter-form editing, scoring/training governance, deform/weight-painted rigs, and a second software host remain later milestones.
...
- Goals: show what has completed, what Back will undo, what Next will execute, and what remains locked; keep the task hierarchy readable; let users inspect and explicitly accept or reject an AI-authored plan without scene mutation; let a user add stable references from active or proposed nodes to one revision-request composer; preserve exact linear turn lineage; show Plan/node/field/simple-parameter changes before acceptance; return every requested change as a reviewable full Plan revision; preserve a single hide/show control.
- Non-goals: an embedded model or automatic task-decomposition engine, streaming assistant chat or a complete history workspace, editable parameter forms, revision branching/merging, in-place mutation of an accepted/proposed Plan, applying a partial JSON patch directly in Blender, automatic Eval scoring or data sanitization, arbitrary built-in UI-element detection, arbitrary MCP execution, native Blender Undo integration, deform rigs/weight painting or arbitrary animation editing, and cross-host parity in this milestone.
- Success signals: a referenced revision request contains the exact base Plan, stable node IDs plus display numbers, user message, thread ID, turn and parent; exact retries are idempotent; a continued turn must use the parent Proposal as its exact base and cannot branch silently; an MCP client can list pending requests and attach one to a strictly newer full GuideProposal with a deterministic diff; Blender shows the diff before acceptance; request submission and replan creation do not mutate the scene or active session; the replacement proposal still requires explicit in-host acceptance; replay export preserves target, exact catalog, proposal, diff, decision, observation and rollback with a stable cursor and deterministic content hash; Blender 4.5 LTS and 5.1 tests pass.
...
- Core routes/screens: connection status, pending proposal identity/thread/diff and decision, safety option, walkthrough controls, guidance visibility, current Back/Next status, revision-request composer with next-turn context, proposal tree, and accepted task tree.
- Content hierarchy: connection first; pending proposal identity, thread, diff and Accept/Reject decision second; safety and accepted-plan Start/Back/Next controls third; guidance visibility fourth; current status fifth; revision-request composer before the trees it references; proposed tree before accepted tree; detailed protocol/diagnostic text last.
...
- New/changed components: shared guidance-state derivation; shared visual tokens; state-aware tree rows; node `Ref` operators for active and proposed trees; one revision-request composer with base-plan/next-turn label, reference summary, message field, Clear and Send controls; Back/Next status controls; numbered viewport badges; haloed colored guide lines and arrows; semantic menu-path fallback; unified Show/Hide Guidance behavior; proposal identity/thread/diff summary; read-only proposal tree; Accept Plan and Reject Plan operators.
- Variants and states: accepted walkthrough steps use `completed`, `back`, `next`, and `locked`. Proposal lifecycle uses `available`, `accepted`, and `rejected`; request-linked proposals additionally use `diff_available`. Revision requests use `empty`, `draft`, `queued`, `acknowledged`, and `error`, with `new_thread` or `continue_thread` context; queued/acknowledged requests never imply a model has replied. A future asynchronous executor may add transient `current`; synchronous Blender operators do not pretend to display a state that cannot be rendered during execution.
- Token/component ownership: host-neutral step/anchor, proposal/decision, node-reference, revision-thread, Plan-diff, revision-request, and Eval bundle semantics stay in `packages/protocol`; proposal/revision-request persistence, thread uniqueness, idempotency, and stable event sequencing stay in `packages/persistence`; thread continuity validation, deterministic diff computation, Eval relation filtering, pagination, summaries, and content hashing stay in the Orchestrator; deterministic accepted-session, proposal-preview, diff validation and local composer lineage stay in Blender application code; Blender RGBA and drawing constants stay in Blender presentation/infrastructure code.
...
- Proposal/request constraints: proposal delivery shares the existing authenticated loopback poll; proposal, decision, revision-thread, Plan-diff, and revision-request payloads are strict, versioned, persisted, and idempotent per companion instance. A request carries the complete immutable base Plan so bundled/offline-origin plans can be replanned without an undocumented server cache. A continued turn must point to the current head and exactly reuse its linked Proposal Plan; current persistence rejects duplicate turns and parents. The background thread handles HTTP only; reference selection, request/thread construction, plan/diff validation, preview-session construction, acceptance, rejection, and session replacement occur on Blender's main thread.
...
- Test/screenshot expectations: targeted protocol/persistence/integration coverage for request idempotency, node/base-plan validation, thread continuity/no silent branch, deterministic diff ordering/content, pending MCP listing, and replan-to-proposal linkage; Blender coverage proving node reference, multi-turn request submission and diff preview do not execute actions or replace sessions; Blender 4.5/5.1 regressions for armature binding, keyframe creation, selected-frame rendering, failure compensation, and full rollback; real GUI captures for a populated revision composer and linked replan proposal with visible diff in addition to existing visual states.
