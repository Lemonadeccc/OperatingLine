...
- Evidence reviewed: `README.md`, `docs/architecture/overview.md`, `docs/architecture/blender-companion.md`, `packages/protocol/src/guide.ts`, `packages/protocol/src/proposal.ts`, `packages/protocol/src/catalog.ts`, `services/orchestrator/src/index.ts`, `packages/persistence/src/index.ts`, `adapters/blender/catalog/v1/action-catalog.json`, `adapters/blender/extension/operating_line/application/companion.py`, `adapters/blender/extension/operating_line/application/session.py`, `adapters/blender/extension/operating_line/presentation/panel.py`, `adapters/blender/extension/operating_line/presentation/operators.py`, `adapters/blender/extension/operating_line/infrastructure/overlay.py`, `adapters/blender/extension/operating_line/resources/snowman.plan.json`, `tests/e2e/blender/capture_overlay.py`, and `docs/assets/blender-guidance.png`.
- Product boundary: the shipped baseline is a deterministic 13-step Blender snowman vertical slice plus model-neutral ActionCatalog/PlanningContext and human-approved GuideProposal. The current delivery target adds node references and immutable revision requests: a Blender user may reference nodes in an active or proposed tree and queue a change request for Codex, Claude, or another MCP client. OperatingLine does not embed a model or stream a chat response; a replan always returns as a complete new GuideProposal revision. Training/Eval export, rigging/animation, and a second software host remain later milestones.
...
- Non-goals: an embedded model or automatic task-decomposition engine, streaming assistant chat, in-place mutation of an accepted/proposed Plan, applying a partial JSON patch directly in Blender, arbitrary built-in UI-element detection, arbitrary MCP execution, native Blender Undo integration, animation, and cross-host parity in this milestone.
...
- Token/component ownership: host-neutral step/anchor, proposal/decision, node-reference, and revision-request semantics stay in `packages/protocol`; proposal/revision-request persistence and idempotency stay in `packages/persistence`; deterministic accepted-session, proposal-preview, and local composer state stay in Blender application code; Blender RGBA and drawing constants stay in Blender presentation/infrastructure code.
