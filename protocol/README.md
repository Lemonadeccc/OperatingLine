...
`procedureMaterialization` 声明可以把轨迹变为 available。Blender InteractionCatalog `1.14.0` 的 UV Sphere
...
`1.14.0` 保留 Icosphere 的六步 ordered menu：四步 guidance 后依次绑定 Location 与 Object Name，
...
另行 opt in 六步 ordered menu：四步 guidance 的 operator step 以 identity 投影绑定 accepted action 的
完整边长 `size`，随后依次绑定 Location 与 Object Name，并显式省略 `resourceId`。`size` 不是 transform
scale；Cube shortcut 与 MCP 均 unavailable。UV Sphere shortcut 结果格式为 `1.2.0`，Icosphere/Cube
menu-only 结果格式为 `1.1.0`；历史 `1.13.0` 精确保留 Icosphere 结果，`1.11.0` ordered menu 返回 `1.1.0`，
`1.10.0` 四步算法与 `1.9.0` unavailable 回放继续返回 `1.0.0`。所有投影仍为 candidate，不证明宿主状态
等价；通用 compile 仍报告 `structural_only`。Cube 轨迹没有真实 Blender replay，原生菜单/operator 不复现
...
`docs/adr/0050-cube-ordered-menu-materialization.md`。
