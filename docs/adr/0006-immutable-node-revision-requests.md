...
当前输入区不是伪装的流式聊天，也没有内置模型。它是一次性的不可变修订请求；模型在外部 MCP
客户端中运行，返回一个完整 Proposal。连续多轮对话可以由客户端基于多条请求实现，但协议不会把
未持久化的聊天上下文当作计划事实。
...
- 当前 UI 使用 Blender 原生单行字符串属性；尚无连续聊天记录、Plan diff 或参数表单编辑器。
- Eval 导出可以直接消费这些持久事件，但导出格式、评分和数据脱敏仍是后续里程碑。
