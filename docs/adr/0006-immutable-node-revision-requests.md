...
但仍不会把未持久化的聊天上下文当作计划事实，见 ADR 0009。ADR 0010 后续从这些规范化事实派生
可分页历史，不复制一份可变聊天日志。
...
- 当前 UI 使用 Blender 原生单行字符串属性；ADR 0009 已增加线性多轮 thread 与 Plan diff，ADR 0010
  已增加结构化历史；尚无实时聊天、分支/合并或参数表单编辑器。
