...
主线程或动作执行已就绪；lease 当前约束 Guide/state 通道和 replay Proposal decision，不替代全局 bearer
和端点 payload 校验。
...
GuidePlan，再调用 `operatingline.planning.evaluate`。当前 Blender `1.20.0` 目录提供十九项适配器自有
...
`1.30.0` 精确绑定 ActionCatalog `1.20.0`，冻结 `1.29.0`，并为 UV Sphere 声明七步 ordered menu 与六步 candidate shortcut：`keyMode` 区分 chord/sequence，
...
Subdivide 保留 managed copy/swap `semantic_path`，并声明 candidate-only 九步 shortcut：`TAB` 进入 Edit
Mode，`A` 选择全部元素，`F3` 输入 literal query `subdivide` 后以 `ENTER` 执行 `mesh.subdivide`，`F9` 打开 Adjust Last
...
`profile` 写入 Width、Segments、Profile Shape，最后以 `ENTER` 和 `TAB` 关闭 surface 并返回 Object Mode。
`targetId`、`resultMeshId`、`resultMeshName` 显式省略；menu/MCP unavailable，因为原生路径就地修改当前
Mesh，不能表达 managed replacement identity 与 transaction。
Individual Inset Faces 保留 managed replacement Mesh `semantic_path`，并声明 candidate-only 十步 shortcut：
`TAB` 进入 Edit Mode、`3` 切换 Face Select、`A` 选择全部面、`I` 以内嵌 `ENTER` 确认完整来源
默认值、`F9` 打开 `mesh.inset` 的 Adjust Last Operation，随后按序把 accepted `thickness`、`depth`
和 literal `true` 写入 Thickness、Depth、Individual，最后以 `ENTER` 和 `TAB` 关闭 surface 并返回 Object Mode。
`targetId`、`resultMeshId`、`resultMeshName` 显式省略；menu/MCP unavailable，因为原生路径就地修改当前
Mesh，不能表达 managed replacement identity 与 transaction。
Poke Faces 保留 managed replacement Mesh `semantic_path`，并声明 candidate-only 九步 shortcut：
`TAB` 进入 Edit Mode、`3` 切换 Face Select、`A` 选择全部面、`F3` 携带 literal query
`poke faces`、`ENTER` 以 UI 默认 `offset=0`、`use_relative_offset=false`、
`center_mode=MEDIAN_WEIGHTED` 执行 `mesh.poke`，`F9` 打开 Adjust Last Operation，Offset 绑定 accepted
`offset`，最后以 `ENTER` 和 `TAB` 关闭 surface 并返回 Object Mode。`targetId`、`resultMeshId`、
`resultMeshName` 显式省略；menu/MCP unavailable。现代默认 keymap 没有 Poke Faces 直达键，legacy
`Alt+P` 不进入默认轨迹。
...
`rotation`、`scale: [1,1,1]`，然后绑定中点 Location 与 Object Name。其封闭
`derived_action_arguments`/`segment_frame` 对 `start`/`end` 计算 `dx/dy/dz`、
`horizontal = hypot(dx,dy)`、`distance = hypot(horizontal,dz)`、`midpoint = (start+end)/2` 与
`rotation = [0,atan2(horizontal,dz),horizontal===0?0:atan2(dy,dx)]`，并把 `-0` 规范化为 `0`。
`distance`、`midpoint` 和 `rotation_euler_xyz_align_z` 必须各恰好映射一次。该 canonical
zero-roll XYZ Euler 不声称与 managed executor quaternion/roll 精确等价；`resourceId` 省略，
shortcut/MCP unavailable。
Cylinder 同样声明六步 ordered menu：四步 guidance operator 严格按序输出
`vertices: 32`、`radius ← radius`、`depth ← distance`、`end_fill_type: NGON`、
`calc_uvs: false`、`enter_editmode: false`、`align: WORLD`、`location: [0,0,0]`、canonical
`rotation`、`scale: [1,1,1]`，然后绑定中点 Location 与 Object Name。它复用同一封闭
...
全部 quad、6/30 结果面选择和 ±1.1 坐标边界，Mesh data pointer 与 datablock 数量保持不变。
Harness 同样只证明已发送 popup 关闭事件；原生 in-place mutation 仍不等价于 managed
replacement Mesh、receipt、Observation、补偿或原生 Undo。Poke Faces 的九步真实前台事件回放在两个
...
validation/materialization，只接受单一 UV Sphere、Icosphere、Cube 或 Plane action、`rollback_step`
成功门、补偿回退和参数完全匹配的 `uv_sphere_ready`、`icosphere_ready`、`cube_ready` 或
`plane_ready`；Icosphere 验证细分级别对应的精确拓扑，Cube/Plane 验证尺寸对应的坐标与固定拓扑。完整 request、
...
ENTER`；`1.24.0` 声明 Edit Mode Bevel 的 `Ctrl+B → F9 → Width → Segments → Profile → ENTER` 参数链；
...
节点局部重规划对当前 Blender `1.20.0` 使用独立的 `ReplanningPromptPacket 1.1.0`；历史目录继续使用
`1.0.0`。MCP
