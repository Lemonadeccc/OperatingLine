...
`1.18.0` 为 UV Sphere 声明七步 ordered menu 与六步 candidate shortcut：`keyMode` 区分 chord/sequence，
...
原生菜单/operator 不复现 managed collection 归属、resource tag、receipt、幂等或补偿语义；Cone/Cylinder 的
Blender 4.5/5.1 operator 双版本探针也不是六步 UI replay。历史
`1.9.0` 至已冻结的 `1.17.0` 继续精确回放，结果格式依实际使用能力为
`1.0.0`/`1.1.0`/`1.2.0`，其中 Icosphere/Cube/Plane/Torus/Cone/Cylinder menu-only 结果为
`1.1.0`。该入口
...
[ADR 0054](../adr/0054-cylinder-segment-frame-menu-materialization.md)。更多 action 的封闭声明、verified
