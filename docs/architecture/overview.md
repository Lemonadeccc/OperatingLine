...
GuidePlan，再调用 `operatingline.planning.evaluate`。当前 Blender `1.18.0` 目录提供十九项适配器自有
...
`1.28.0` 精确绑定 ActionCatalog `1.18.0`，冻结 `1.27.0`，并为 UV Sphere 声明七步 ordered menu 与六步 candidate shortcut：`keyMode` 区分 chord/sequence，
...
节点局部重规划对当前 Blender `1.18.0` 使用独立的 `ReplanningPromptPacket 1.1.0`；历史目录继续使用
`1.0.0`。MCP
