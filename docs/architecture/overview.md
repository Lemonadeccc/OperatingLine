...
`1.12.0` 为 UV Sphere 声明七步 ordered menu 与六步 candidate shortcut：`keyMode` 区分 chord/sequence，
...
hash 与逐 leaf coverage；MCP 仍因没有获批 action-level tool 而 unavailable。leaf 仍为 candidate 且
`validatedHostVersions` 为空，通用 compile 仍只报告 `structural_only`。radius→scale 与相对移动是教学投影，
不是宿主状态等价证明。历史 `1.9.0`/`1.10.0`/`1.11.0` 继续精确回放，结果格式依实际使用能力为
...
[ADR 0048](../adr/0048-candidate-shortcut-procedure-materialization.md)。verified shortcut/MCP recipe、真实
