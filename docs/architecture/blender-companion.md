...
`1.4.0` 与 ActionCatalog `1.7.0` 一一绑定 15 个 action，并由活动叶节点的 `actionName` 选择配方。
...
三个身体球；Cube、鼻子和手臂会分别切换到各自 recipe。批量几何、材质、骨骼、动画和渲染等八条
...
`blender + instanceId + ActionCatalog 1.7.0`、原始目标和一个新 Plan ID，不包含 Provider 或凭据。
...
当前动作目录 `1.7.0` 允许以下 15 类 action，把它们完整划分到 Geometry、Materials、Animation、
Render setup 和 Output 五个有序规划阶段，并保留不可变 `1.0.0`、`1.1.0`、`1.2.0`、`1.3.0`、`1.4.0`、`1.5.0`、`1.6.0` 供精确回放：
...
`1.7.0` 保留 `1.3.0` 首次发布的七项适配器自有 `semanticCapabilities`：ground plane、primitive assembly、Principled
