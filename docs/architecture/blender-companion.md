...
`1.11.0` 与 ActionCatalog `1.12.0` 一一绑定 22 个 action，并由活动叶节点的 `actionName` 选择配方；历史
`1.9.0`/`1.10.0` 保持精确回放。`1.11.0` 只为 UV Sphere 声明目录绑定的 Procedure menu 与 Location、
Scale、Object Name 有序控件物化；shortcut/MCP 仍明确 unavailable，radius→scale 仍只是 candidate 教学投影。
