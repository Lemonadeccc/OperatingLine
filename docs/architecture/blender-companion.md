...
`1.32.0` 与 ActionCatalog `1.22.0` 一一绑定 27 个 action，并由活动叶节点的 `actionName` 选择配方；历史
`1.9.0` 至已冻结的 `1.31.0` 保持精确回放。UV Sphere 保留目录绑定的七步 menu 与六步
...
unavailable。原生 operator 就地修改当前 Mesh，不创建 managed replacement identity。
Individual Inset Faces 也保留 managed replacement Mesh `semantic_path`，并增加 candidate-only
十步 shortcut：`TAB → 3 → A → I → F9 → Thickness → Depth → Individual true → ENTER → TAB`。
`I` 步骤固定完整来源 operator 默认值并以内嵌 `ENTER` 确认；F9 控件按序绑定
accepted `thickness`、`depth` 和 literal `use_individual=true`。`targetId`、`resultMeshId`、
`resultMeshName` 显式省略，menu/MCP unavailable。原生 operator 就地修改当前 Mesh，
不创建 managed replacement identity。
Poke Faces 同样保留 managed replacement Mesh `semantic_path`，并增加 candidate-only 九步
shortcut：`TAB → 3 → A → F3(query="poke faces") → ENTER → F9 → Offset → ENTER → TAB`。
搜索执行步骤固定 UI 默认 `offset=0`、`use_relative_offset=false` 与
`center_mode=MEDIAN_WEIGHTED`，F9 控件再把 accepted `offset` 写入 `mesh.poke.offset`。
`targetId`、`resultMeshId`、`resultMeshName` 显式省略，menu/MCP unavailable；现代默认 keymap 没有
Poke Faces 直达键，legacy `Alt+P` 不进入默认轨迹。原生 operator 就地修改当前 Mesh，不创建 managed
replacement identity。
Mirror Modifier 保留七步 `semantic_path`：`Layout → Owned Mesh → Modifiers → Add Modifier → Generate →
Mirror → Managed Mirror Contract`。其中 Add Modifier、Generate 和 Mirror 分别记录真实 Host 标识
`OBJECT_MT_modifier_add`、`OBJECT_MT_modifier_add_generate` 与 `object.modifier_add(type=MIRROR)`。
Properties Modifier context 的 `Shift+A` 只是一条打开 Add Modifier 的上下文别名，不能携带 IDs、
受管名称、完整固定 RNA、安全门、Observation 或回退；menu/shortcut/MCP materialization 均 unavailable。
Icosphere、Cube、Plane、Torus、Cone 和 Cylinder 都省略内部
...
`1.2.0`，Subdivide shortcut 使用 Result `1.3.0`，其余这些 menu-only 结果使用 Result `1.1.0`；轨迹仍是
...
receipt、Observation 与补偿事务。Edit Mode Bevel 的十步轨迹在两个版本均验证 `width=0.2`、
`segments=3`、`profile=0.6`，得到 96 vertices、192 edges、98 polygons，且源 Mesh pointer/datablock
数量不变；harness 只证明已经发送 popup 关闭事件。该原生 in-place mutation 不等价于 managed
copy/tag/swap、receipt、Observation、补偿或原生 Undo transaction。Individual Inset Faces 的十步轨迹在两个版本均验证
`thickness=0.2`、`depth=0.1`、`use_individual=true`，得到 32 vertices、60 edges、30 polygons、
全部 quad、6/30 结果面选择和 ±1.1 坐标边界，且源 Mesh pointer/datablock 数量不变。
Harness 同样只证明已发送 popup 关闭事件；该原生 in-place mutation 不等价于 managed
copy/tag/swap、receipt、Observation、补偿或原生 Undo transaction。Poke Faces 的九步轨迹在两个版本
均验证 `offset=0.2`，Cube 从 8/12/6 变为 14/36/24，24 个结果面全部为 selected triangle，坐标边界为
±1.2，且源 Mesh pointer/datablock 数量不变。UI RNA 的 `MEDIAN_WEIGHTED` 与 managed BMesh 路径的
`MEAN_WEIGHTED` 是不同接口的字面量；该 UI in-place mutation 同样不等价于 managed transaction。
Mirror 的 Properties/Shift+A 前台轨迹在两版均创建 `OBJECT_OT_modifier_add(type=MIRROR,
use_selected_objects=false)`，Cube 求值拓扑从 8/12/6 变为 16/24/12，源 Mesh pointer 与 datablock
数量不变；这只证明原生别名存在，不升级为 candidate 或 managed execution。
Cube 与
...
为 Torus 增加 `torus_ready`，`1.22.0` 为 Cone/Cylinder 增加 `cone_ready`/`cylinder_ready`。七者都从
...
`8/12/6` 与 `4/4/1`；Torus 按两组 segments 验证动态拓扑，并按 accepted radii 逐顶点复算参数化坐标；
Cone/Cylinder 复算中点、方向 Quaternion、世界端点、32 段局部环坐标和面形，区分 `64/96/34` 锥台/圆柱与
`33/64/33` 单端尖锥。七者都拒绝额外
...
ActionCatalog `1.22.0` 和能力画像；只有 Runtime 返回匹配目录与当前 Guide `1.5.0` 后，UI 才显示
...
`blender + instanceId + ActionCatalog 1.22.0`、原始目标和一个新 Plan ID，不包含 Provider 或凭据。
...
当前动作目录 `1.22.0` 允许以下 27 类 action，把它们完整划分到 Geometry、Materials、Animation、
Render setup 和 Output 五个有序规划阶段，并保留不可变 `1.0.0` 至 `1.21.0` 供精确回放：
...
`1.22.0` 提供十九项适配器自有 `semanticCapabilities`：ground plane、primitive assembly、whole-mesh
...
Edit/Modifier/Geometry Nodes 当前开放十一个有界动作：Subdivide 复制源 Mesh、对整网格执行
...
实际 level 的投影输出应用 8192/16384/8192 topology 上限；Edit Mode Bevel 只接受 `width`
(`0.0001..100`)、`segments` (`1..16`) 与 `profile` (`0..1`)，要求 Object Mode、无 Modifier/Shape Key、
每条边恰好连接两个面的非空 closed manifold Mesh；它固定其余 BMesh Bevel 属性，对全部边生成受
8192/16384/8192 上限约束的 replacement Mesh，并要求三项 topology 严格增长；Individual Inset Faces
接受 `targetId`、`resultMeshId`、`resultMeshName`、`thickness: 0.0001..100` 与 `depth: -100..100`，
要求 receipt-owned Object Mode Mesh、无 Modifier/Shape Key、非空 closed manifold 且所有面面积有限为正。
它对全部面固定调用 `bmesh.ops.inset_individual`，其中 `use_even_offset=true`、
`use_interpolate=true`、`use_relative_offset=false`。若面环总数为 `L`，结果精确为
`V+L / E+2L / F+L`；closed manifold 下 `L=2E`，等价于 `V+2E / 5E / F+2E`。Poke Faces 只接受
`offset: -100..100`，要求相同的 closed nondegenerate manifold 且每个顶点形成单一 manifold face fan，并固定
`center_mode=MEAN_WEIGHTED`、`use_relative_offset=false`。它把全部源面转换为三角扇；若面环总数为
`L`，结果精确为 `V+F / E+L / L`，closed manifold 下等价于 `V+F / 3E / 2E`。两种动作的结果都受相同
8192/16384/8192 上限约束。Poke 使用专用 `mesh_faces_poked` Observation、source/result
`mesh_content` guard 和独立 Blender 原生 Undo/Redo 回归。Mirror 只接受 `targetId`、`modifierId`、
受管名称和单一 `X/Y/Z` 轴；要求 Object Mode、无 Shape Key、全部前置 Modifier 均 receipt-tracked 且
不存在 MIRROR。输入、两倍保守投影和真实 evaluated output 均受 8192/16384/8192 上限约束；merge 固定
开启且阈值为 0.001，bisect/flip/clip/UV/UDIM 固定关闭，offsets 归零，Mirror Object 为空，
vertex-group mirroring 和 viewport/render/edit 可见性开启。receipt 用完整 ModifierState、Object data
同值 guard 与源 `mesh_content` guard 同时保护 RNA、绑定和内容；严格 `modifier_ready` Observation 与
原生 Undo/Redo 都在 Blender 4.5.3/5.1.1 验证。Geometry Nodes 创建固定的 Group Input → Transform Geometry → Group Output 图并通过一个 NODES modifier
