...
- **目录绑定的 Procedure 轨迹物化**：供应商无关的 MCP
  ...
  validation。只有 InteractionCatalog 的封闭声明可启用轨迹。Blender InteractionCatalog `1.13.0` 为
  UV Sphere 生成七步菜单，
  ...
  仍因没有真实 action-level tool 而 unavailable。旧 `1.10.0` 四步菜单、`1.11.0` 七步菜单和 `1.12.0`
  UV Sphere 快捷键结果保持精确回放。结果带已安装目录 digest、输入/输出 tree hash 与逐 leaf coverage；最新快捷键结果格式为
  `1.2.0`。leaf 仍为 candidate 且 `validatedHostVersions` 为空，通用 compile 仍是 `structural_only`；
  radius→scale 与相对移动只是教学投影，不是宿主状态等价证明。历史 `1.9.0`/`1.10.0`/`1.11.0`/`1.12.0` 保持可回放。只有完整 result 信封保留目录
  ...
  [ADR 0049](docs/adr/0049-icosphere-ordered-menu-materialization.md)。
...
- **InteractionCatalog**：通用协议把一个已接受 action 映射为版本化、有序的宿主交互步骤，并区分
  可绑定真实控件的 `native_path` 与只供教学参考的 `semantic_path`。Blender InteractionCatalog `1.13.0` 精确绑定
  ...
  `UI target unavailable`，不会复用无关按钮。历史 `1.9.0`/`1.10.0`/`1.11.0`/`1.12.0` 保持逐字可回放。见
