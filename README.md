...
> 回退一张确定性的雪人渲染预览。
> Orchestrator 现在可以查询 Blender `1.18.0` ActionCatalog 和 PlanningContext，并导出带冻结快照游标
...
> Blender 4.5/5.1 中真实执行的机器人基准。当前 `1.18.0` 目录提供十九项 `semanticCapabilities`，要求
...
- **快捷键 Operator 参数 surface**：协议可显式记录 `F9` opener、逐控件
  ...
  `1.3.0`；Schema 14 的精确索引可按共享 surface/operator 查询完整链。当前 InteractionCatalog `1.28.0`
...
- **目录绑定的 Procedure 轨迹物化**：供应商无关的 MCP
  ...
  validation。只有 InteractionCatalog 的封闭声明可启用轨迹。Blender InteractionCatalog `1.28.0` 为
  UV Sphere 生成七步菜单，
...
- **InteractionCatalog**：通用协议把一个已接受 action 映射为版本化、有序的宿主交互步骤，并区分
  ...
  [ADR 0063](docs/adr/0063-bounded-edit-mode-poke-faces.md) 与
  [ADR 0064](docs/adr/0064-bounded-mirror-modifier.md)。
- **跨目标规划质量门与需求覆盖证据**：Blender catalog `1.18.0` 把 27 个动作划分为 Geometry、Materials、
