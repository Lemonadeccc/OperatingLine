...
> 回退一张确定性的雪人渲染预览。
> Orchestrator 现在可以查询 Blender `1.22.0` ActionCatalog 和 PlanningContext，并导出带冻结快照游标
...
> Blender 4.5/5.1 中真实执行的机器人基准。当前 `1.22.0` 目录提供十九项 `semanticCapabilities`，要求
...
> 有界整网格 Subdivide/Triangulate/Edit Mode Bevel/Individual Inset Faces/Poke Faces、显式连通面区域 Extrude、非应用 Bevel/Solidify/Subdivision Surface/Mirror Modifier、首个 Transform Geometry Nodes、显式归一化蒙皮权重和
...
- **目录绑定的 Procedure 轨迹物化**：供应商无关的 MCP
  ...
  各生成六步 ordered menu：四步原生
  guidance 的最终 operator 以 identity 投影绑定 accepted action 的 `size`，随后依次绑定 Location 与 Object
  Name；`size` 是完整边长，不是 Blender transform scale，内部 `resourceId` 同样省略。Cube 与 Plane
  各有严格 `candidate_only` 的六步 shortcut：在 `Layout`、`VIEW_3D`、`OBJECT`、Blender keymap、
  3D Cursor `[0,0,0]`、GLOBAL Transform Orientation 六项前置条件下，依次执行
  `Shift+A → Mesh → Cube` 或 `Shift+A → Mesh → Plane`（默认 `size: 2`、`location: [0,0,0]`）、
  `G X`、`G Y`、`G Z`、
  `S` 与 `F2`；三次移动分别以
  `vector3_x/y/z` 绑定 accepted `location`，`S` 通过封闭 `divide_by_two` 绑定 `size / 2`，`F2`
  绑定 `objectName`。Cube 与 Plane 的 `resourceId` 均显式省略，MCP unavailable。Torus 同版本在
  operator step 按顺序绑定 `major_segments`、`minor_segments`、literal
  ...
  不声称与 managed executor 的 quaternion/roll 精确等价。Cone 同样省略 `resourceId`，shortcut/MCP 均 unavailable。
  Cylinder 同版本生成六步 ordered menu：四步原生 guidance 的 operator step 严格按序绑定
  `vertices: 32`、`radius ← radius`、`depth ← distance`、`end_fill_type: NGON`、
  `calc_uvs: false`、`enter_editmode: false`、`align: WORLD`、`location: [0,0,0]`、
  `rotation` 与 `scale: [1,1,1]`，然后绑定中点 Location 与 Object Name。它复用同一封闭
  `segment_frame` 公式及三个输出各恰好一次的约束；canonical zero-roll XYZ Euler 将本地
  `+Z` 对齐 `end-start`，本地 `-Z` 端对应 `start`，本地 `+Z` 端对应
  `end`，两端使用同一 `radius`。该 Euler 同样不声称与 managed executor 的
  quaternion/roll 精确等价；`resourceId` 省略，shortcut/MCP 均 unavailable。Subdivision Surface Modifier
  另有四步 candidate shortcut：`Ctrl+1` 以 literal `level=1`、`relative=false`、
  `ensure_modifier=true` 创建默认 modifier，`F9` 打开 `object.subdivision_set` 的 Adjust Last Operation，
  `Level` 绑定 accepted `viewportLevel`，再以 `ENTER` 结束 surface。`targetId`、`modifierId`、
  `modifierName` 显式省略，menu/MCP unavailable；目标版本中的 `F3` 搜索路径实测不可用，因此未收录。
  Edit Mode Bevel 另有十步 candidate shortcut：`TAB → 2 → A → Ctrl+B → F9 → Width → Segments →
Profile → ENTER → TAB`。`Ctrl+B` 固定完整来源 operator 默认值并以内嵌 `ENTER` 确认，三个 F9 控件
  依次绑定 accepted `width`、`segments`、`profile`；`targetId`、`resultMeshId`、`resultMeshName` 显式
  ...
  Poke Faces 同样保留 managed replacement Mesh `semantic_path`，并提供九步 candidate shortcut：
  `TAB → 3 → A → F3(query="poke faces") → ENTER → F9 → Offset → ENTER → TAB`。执行步骤保存完整
  UI RNA 默认值 `offset=0`、`use_relative_offset=false`、`center_mode=MEDIAN_WEIGHTED`，F9 控件把
  accepted `offset` 写入准确数组位置；managed BMesh 则固定使用不同接口的字面量 `MEAN_WEIGHTED`。
  `targetId`、`resultMeshId`、`resultMeshName` 显式省略，menu/MCP unavailable。两版真实回放均把
  Cube 从 8/12/6 变为 14/36/24 个全 triangle topology，24 个结果面均被选择、坐标边界为 ±1.2，
  并保持 Mesh pointer/datablock 数量不变。
  现代默认 keymap 没有 Poke Faces 直达键，legacy `Alt+P` 不进入默认训练轨迹。
  Mirror Modifier 另保留七步语义路径：`Layout → Owned Mesh → Modifiers → Add Modifier → Generate →
Mirror → Managed Mirror Contract`。Blender 4.5.3/5.1.1 的真实前台事件回放证明在 Properties 的
  Modifier context 中 `Shift+A` 可打开 `OBJECT_MT_modifier_add`，再从 Generate 选择 Mirror 并执行
  `object.modifier_add(type=MIRROR)`；两版都得到 Cube `8/12/6 → 16/24/12`，源 Mesh pointer 与
  datablock 数量不变。该别名无法携带 logical IDs、受管名称、固定 RNA、拓扑门、Observation 与回退，
  因而 menu、shortcut 和 MCP procedure materialization 均保持 unavailable。
  UV Sphere、Cube 与 Plane shortcut 结果格式为 `1.2.0`；Icosphere、Subdivide、Subdivision Surface、
  Edit Mode Bevel、Individual Inset Faces 与 Poke Faces shortcut 结果格式为 `1.3.0`；Torus、Cone 与
  Cylinder 的 menu-only 结果格式为 `1.1.0`。旧 `1.10.0` 四步菜单、`1.11.0` 七步菜单、`1.12.0` UV Sphere
  ...
  `1.9.0` 至已冻结的 `1.31.0` 保持可回放；只有完整 result 信封保留目录
  ...
  Blender replay。Cube 与 Plane 的 Blender 4.5.3/5.1.1 operator/transform 双版本探针不是实际键盘事件或
  完整 UI replay；它们保留默认未 bake mesh 与 `scale = size / 2`，不同于 managed executor 的 baked
  mesh/`scale = 1`。Cone/Cylinder
  的原生 operator 双版本探针也不等于六步 UI replay。原生菜单/operator 不能复现 action executor 的 managed
...
- **受管 Procedure 叶节点回放证明**：MCP/HTTP 的 `procedure.replay.propose` 对原始 packet + candidate
  ...
  单一 UV Sphere、Icosphere、Cube、Plane、Torus、Cone 或 Cylinder 叶节点、`rollback_step` 成功门，以及与 Action 参数完全一致的
  `uv_sphere_ready`、`icosphere_ready`、`cube_ready`、`plane_ready`、`torus_ready`、`cone_ready` 或
  `cylinder_ready` Observation；Icosphere 把
  `subdivisions` 绑定到精确拓扑，Cube/Plane 把 `size` 绑定到精确坐标和固定拓扑，Torus 则把两组 segments 与
  radii 绑定到逐顶点局部坐标和动态拓扑；Cone/Cylinder 复算中点、方向 Quaternion、世界端点、32 段局部环
  坐标和普通/尖锥拓扑。用户在 Blender 接受
  ...
  `candidate_not_executed`，Torus/Cone/Cylinder 等无候选时为 `unavailable`；MCP 保持 `unavailable`。它不是逐控件 UI replay，也不单独证明 Blender 原生
  ...
  [ADR 0069](docs/adr/0069-segment-primitives-managed-replay-attestation.md)。
...
- **InteractionCatalog**：通用协议把一个已接受 action 映射为版本化、有序的宿主交互步骤，并区分
  ...
  [ADR 0063](docs/adr/0063-bounded-edit-mode-poke-faces.md) 与
  [ADR 0064](docs/adr/0064-bounded-mirror-modifier.md)。
- **跨目标规划质量门与需求覆盖证据**：Blender catalog `1.22.0` 把 27 个动作划分为 Geometry、Materials、
...
- **完整雪人预览垂直切片**：内置 revision 6 教学计划包含 7 个阶段、25 个可执行步骤，依次完成
  ...
  整条执行链。
- **有界 Edit/Modifier/Geometry Nodes 切片**：ActionCatalog 另提供整网格 Subdivide/Triangulate/Edit Mode Bevel/Individual Inset Faces/Poke Faces、显式连通面区域 Extrude、非应用
  Bevel/Solidify/Subdivision Surface/Mirror Modifier 和固定 Transform Geometry Nodes 图十一个动作。Extrude 在读取 polygon index 前验证源 Mesh 内容与 Object→Mesh receipt 链，
  ...
  只开放 `viewportLevel: 1..3`，固定 Catmull-Clark、render level 2 与完整可观察属性，并同时限制输入与
  ...
  因而等价于 `V+2E / 5E / F+2E`。Poke Faces 只接受 `offset: -100..100`，要求每个顶点形成单一
  manifold face fan，并固定
  `center_mode=MEAN_WEIGHTED` 与 `use_relative_offset=false`；它把全部源面转换为三角扇，结果精确为
  `V+F / E+L / L`，closed manifold 下等价于 `V+F / 3E / 2E`。
  Mirror 只开放一个本地 `X/Y/Z` 轴，要求 Object Mode、无 Shape Key、全部前置 Modifier 均有 receipt，
  且不存在既有 MIRROR。它把输入拓扑按最多两倍投影并同时限制真实求值输出；merge 固定开启且阈值为
  `0.001`，bisect/flip/clip、UV/UDIM 与 offsets 固定关闭或归零，Mirror Object 固定为空，vertex group
  mirroring 与 viewport/render/edit 可见性固定开启。源 Mesh 内容和 Object→Mesh 绑定都由同值 guard 保护。
  Mesh、modifier 与 node group 都进入 receipt 和专用 observation；若用户在执行后修改其拓扑、属性或节点图，`Back` 会保留现场与 receipt
...
> `operatingline.planning.evaluate` 后提交 GuideProposal。当前 Blender 目录覆盖 27 个已验证动作和
> 十九项适配器声明的语义能力，
...
Blender 当前允许 27 个版本化 action，覆盖七种单体 Mesh、原子基础体批次、Subdivide、Triangulate、显式连通面区域 Extrude、Edit Mode Bevel、Individual Inset Faces、Poke Faces、Bevel、Solidify、Subdivision Surface、Mirror、
...
有界 Mirror Modifier 的单轴、固定 RNA、拓扑与回退边界见
[ADR 0064](docs/adr/0064-bounded-mirror-modifier.md)。
...
Guide/状态/Proposal decision 通信前协商 ActionCatalog、Guide 协议和能力画像，并以 5 秒心跳续订
15 秒运行时租约；
...
pnpm test:blender:edit-poke
pnpm test:blender:mirror
...
`pnpm test:blender:subdivision-surface` 在 Blender 4.5.3/5.1.1 中分别以 level 1/2/3 回放
`Ctrl+1 → F9 → Level → Enter`，核对 operator/Modifier 完整属性、求值拓扑和源 Mesh 身份不变。
...
`pnpm test:blender:edit-poke` 在两个版本中回放
`TAB → 3 → A → F3 → poke faces → Enter → F9 → Offset → Enter → TAB`，核对完整 operator 属性、
`offset=0.2`、14/36/24 全 triangle topology、24 个结果面选择、±1.2 坐标边界和源 Mesh 身份不变。
`pnpm test:blender:mirror` 在两个版本的真实 Properties Modifier context 中回放
`Shift+A → mirror → Enter`，核对 `OBJECT_OT_modifier_add(type=MIRROR)`、完整默认 RNA、
Cube `8/12/6 → 16/24/12`、源 Mesh pointer 和 datablock 数量不变；该测试只证明原生 UI 别名，
不把它升级为 managed materialization。
...
删除/恢复/冲突拒绝；同一命令还分别独立验证 Individual Inset、Poke Faces replacement Mesh 与 Mirror
Modifier 的删除、恢复和 receipt pointer 重绑定。
