...
- **目录绑定的 Procedure 轨迹物化**：供应商无关的 MCP
  ...
  validation。只有 InteractionCatalog 的封闭声明可启用轨迹。Blender InteractionCatalog `1.14.0` 为
  UV Sphere 生成七步菜单，
  ...
  仍因没有真实 action-level tool 而 unavailable。Cube 同版本生成六步 ordered menu：四步原生 guidance
  的最终 operator 以 identity 投影绑定 accepted action 的 `size`，随后依次绑定 Location 与 Object Name；
  `size` 是完整边长，不是 Blender transform scale，内部 `resourceId` 同样省略，shortcut/MCP 均 unavailable。
  UV Sphere shortcut 结果格式仍为 `1.2.0`，Icosphere 与 Cube 的 menu-only 结果格式为 `1.1.0`。旧
  `1.10.0` 四步菜单、`1.11.0` 七步菜单、`1.12.0` UV Sphere 快捷键与 `1.13.0` Icosphere 菜单结果保持
  ...
  `1.9.0`/`1.10.0`/`1.11.0`/`1.12.0`/`1.13.0` 保持可回放。只有完整 result 信封保留目录
  grounding 证明；单独抽出或经通用 store 保存的 tree 仍只能
  按 `structural_only` 使用。该入口不调用模型或 Provider、不保存树、不创建 Proposal，也不执行 Blender；
  当前 Cube 轨迹也未经过真实 Blender replay。原生菜单/operator 不能复现 action executor 的 managed
  ...
  [ADR 0050](docs/adr/0050-cube-ordered-menu-materialization.md)。
...
- **InteractionCatalog**：通用协议把一个已接受 action 映射为版本化、有序的宿主交互步骤，并区分
  可绑定真实控件的 `native_path` 与只供教学参考的 `semantic_path`。Blender InteractionCatalog `1.14.0` 精确绑定
  ...
  `UI target unavailable`，不会复用无关按钮。历史 `1.9.0`/`1.10.0`/`1.11.0`/`1.12.0`/`1.13.0` 保持逐字可回放。见
